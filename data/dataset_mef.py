...
class DatasetMEF(data.Dataset): 
    ...
    def __getitem__(self, index):
        ...
        if self.opt['phase'] == 'train': 
            ...
            h_GT,w_GT,c_GT = patch_GT.shape 
            
            assert img_A.shape == img_GT.shape,f"Wrong Path: {A_path} \n {GT_path} {img_A.shape}|{img_GT.shape}"
            assert h_GT==w_GT and h_GT!=0 and w_GT!=0, f"Wrong Path: {GT_path} \n Wrong shape: {img_GT.shape}|{patch_GT.shape} \n Wrong location: {rnd_h,rnd_w} \n Wrong size: {self.patch_size}"
