...
class ModelPlain(ModelBase):
    ...
    # ----------------------------------------
    def feed_data(self, data, need_GT=True, phase='test'):
    ...
    def netG_forward_window(self, phase='test', window_size=256, stride=128):
        self.E = torch.zeros_like(self.A)  # 初始化输出张量

        # 计算图像窗口数量
        num_windows_h = (self.A.size(2) - window_size) // stride + 1
        num_windows_w = (self.A.size(3) - window_size) // stride + 1
        ...
        for i in range(num_windows_h):
            for j in range(num_windows_w):
                ...
                # 从输入张量中提取当前窗口
                A_window = self.A[:, :, start_h:end_h, start_w:end_w]
                B_window = self.B[:, :, start_h:end_h, start_w:end_w]

                # 使用当前窗口进行推理
                E_window = self.netG(A_window, B_window)

                # 将推理结果填充回输出张量的对应窗口位置
                self.E[:, :, start_h:end_h, start_w:end_w] = E_window
    ...
    def optimize_parameters(self, current_step):
        ...
        elif G_lossfn_type in ['mef']:
            total_loss, loss_text, loss_int, loss_ssim = self.G_lossfn(self.A, self.B, self.E, self.GT)
            G_loss = self.G_lossfn_weight * total_loss 
       
        elif G_lossfn_type in ['mff', 'vif', 'nir', 'med']:
    ...
    def test(self):
        ...
        with torch.no_grad():
            ...
            self.netG_forward_window(phase='test')
        self.netG.train()
