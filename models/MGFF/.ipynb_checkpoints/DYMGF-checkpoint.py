import os
import sys
sys.path.append("/code/HDR/MGFF")
from MGFF.swinir_backbone import *
from MGFF.shared_dyconv import Mutual_CrossAttention_ODEncoder as Odconv_Dynamic_Encoder
from MGFF.shared_dyconv import test_flops
...
class MGFF_Blocks(nn.Module):
    ...
    def __init__(self, input_dim, dim, input_resolution, depth, num_heads, window_size,
        ...
        self.MGF = Odconv_Dynamic_Encoder(input_dim,dim,kernel_num=kernel_num,inference=inference,mid_channel=mid_channel,temperature=temperature)
...
#     model = MGFF().cuda().eval()
#     height=256
#     width=256
#     rgb = torch.randn((1, 3, height, width)).cuda()
#     depth = torch.randn((1, 1, height, width)).cuda()
